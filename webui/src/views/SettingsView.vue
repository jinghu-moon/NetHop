...
import { Button as TButton, Collapse as TCollapse, CollapsePanel as TCollapsePanel } from "tdesign-mobile-vue";
...
async function load(): Promise<void> {
  ...
  try {
    ...
    uiStores.config.load(config); fields.value = schema.fields; capability.value = new Map(report.items.map((item) => [item.key, { status: item.status, reason: item.reasonCode }])); conflict.value = false;
...
<template>
  <section class="page settings-page">
    ...
    <template v-else>
      ...
      <TCollapse :default-value="groups.filter((group) => group.name !== 'advanced').map((group) => group.name)"><TCollapsePanel v-for="group in groups" :key="group.name" :value="group.name" :header="group.name"><div class="schema-grid"><div v-for="field in group.fields" :key="field.id" class="schema-field-wrap"><SchemaField :field="definition(field)" @change="(value) => updateValue(field.path, value)" /><StatusLine v-if="field.experimental" status="degraded" label="实验功能" /><small>{{ field.applyImpact }} · {{ field.riskLevel }}</small></div></div></TCollapsePanel></TCollapse>
    </template>

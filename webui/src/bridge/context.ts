...
const envelope = (result: unknown): string => JSON.stringify({ version: 2, request_id: "mock", ok: true, result });
...
export function createAppHost(): HostAdapter {
  ...
  return createMockHost({ responses: {
    "status.get": { errno: 0, stdout: envelope({ schema_version: 2, state: "fail_open_direct", generation: null, last_update: "never", watcher_health: {}, runtime: {}, subscription: {}, core_update: {}, rule_set: {}, dns_split: {}, capture: {}, operational: {} }), stderr: "" },
    ...
    "subscription.list": { errno: 0, stdout: envelope({ subscriptions: [
      { id: "src_11111111111111111111111111111111", name: "Primary", enabled: true, url_redacted: "[REDACTED]" },
      { id: "src_22222222222222222222222222222222", name: "Backup", enabled: true, url_redacted: "[REDACTED]" },
    ] }), stderr: "" },
    ...
    "config.schema": { errno: 0, stdout: envelope({ schema_version: 2, fields: [], features: [] }), stderr: "" },
    "capability.get": { errno: 0, stdout: envelope({ schema_version: 2, probe_id: "probe-mock", observed_at_monotonic_ms: 0, report_digest: "0".repeat(64), items: [] }), stderr: "" },
    "node.list": { errno: 0, stdout: envelope({ nodes: [
      { id: "nh1s-0123456789abcdef", name: "自动优选", protocol: "vless", latency_ms: 42, selected: true, source_ids: ["src_11111111111111111111111111111111"] },
      { id: "nh1s-fedcba9876543210", name: "新加坡 · 高速", protocol: "trojan", latency_ms: null, selected: false, source_ids: ["src_11111111111111111111111111111111"] },
      { id: "nh1s-1111111111111111", name: "东京 · 低延迟", protocol: "hysteria2", latency_ms: 31, selected: false, source_ids: ["src_22222222222222222222222222222222"] },
      { id: "nh1s-2222222222222222", name: "洛杉矶 · 备用", protocol: "vmess", latency_ms: 180, selected: false, source_ids: ["src_22222222222222222222222222222222"] },
    ] }), stderr: "" },
    "node.test-all": { errno: 0, stdout: envelope({ results: [
      { tag: "nh1s-0123456789abcdef", delay_ms: 64 },
      { tag: "nh1s-fedcba9876543210", delay_ms: 91 },
      { tag: "nh1s-1111111111111111", delay_ms: 48 },
      { tag: "nh1s-2222222222222222", delay_ms: 188 },
    ] }), stderr: "" },

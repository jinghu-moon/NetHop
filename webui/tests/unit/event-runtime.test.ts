...
describe("bounded reconnect and traffic", () => {
  ...
  it("performs hello before events and stops the child on visibility changes", async () => {
    ...
    const host = createMockHost({
      ...
      streams: { "events.subscribe": ['{"version":5,"request_id":"events","sequence":1,"kind":"item","payload":{"kind":"snapshot","runtime":{}}}\n'] },
      closeStreams: false,
    });
  ...
  it("treats an incompatible hello as terminal and never spawns events", async () => {
    vi.useFakeTimers();
    const host = createMockHost({ responses: { hello: { errno: 0, stderr: "", stdout: JSON.stringify({ version: 5, request_id: "hello", ok: true, result: { manager_version: "webui-0.1.0", compatible: false, daemon_protocol_min: 1, daemon_protocol_max: 1, daemon_schema_min: 1, daemon_schema_max: 1, active_schema_version: 1, supported_operations: [], supported_features: [] } }) } } });
    const session = new EventSession({ host, kinds: ["runtime"], managerVersion: "webui-0.1.0" });
    session.start();
    await vi.runAllTimersAsync();

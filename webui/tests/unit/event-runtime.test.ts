...
const frame = (sequence: number, kind: string, extra: Record<string, unknown> = {}): unknown => ({ version: 6, request_id: "events", sequence, kind: "item", payload: { kind, ...extra } });
...
describe("bounded reconnect and traffic", () => {
  ...
  it("keeps only a fixed 60-point monotonically ordered window", () => {
    ...
    for (let index = 0; index < 86_400; index += 1) ring.push({ up: index, down: index * 2, state: "ok", intervalMs: 1_000, observedAtMs: index === 30 ? 1 : index });
  ...
  it("publishes at most once per animation frame and keeps the newest sample", () => {
    ...
    for (let index = 0; index < 100; index += 1) coalescer.push({ timestampMs: index, up: index, down: index, state: "ok", intervalMs: 1_000, observedAtMs: index });
  ...
  it("performs hello before events and stops the child on visibility changes", async () => {
    ...
    const host = createMockHost({
      responses: {
      hello: { errno: 0, stderr: "", stdout: JSON.stringify({ version: 6, request_id: "hello", ok: true, result: { manager_version: "webui-0.1.0", compatible: true, daemon_protocol_min: 6, daemon_protocol_max: 6, daemon_schema_min: 3, daemon_schema_max: 3, active_schema_version: 3, supported_operations: [], supported_features: [] } }) },
      },
      streams: { "events.subscribe": ['{"version":6,"request_id":"events","sequence":1,"kind":"item","payload":{"kind":"snapshot","runtime":{}}}\n'] },
      closeStreams: false,
    });
  ...
  it("treats an incompatible hello as terminal and never spawns events", async () => {
    vi.useFakeTimers();
    const host = createMockHost({ responses: { hello: { errno: 0, stderr: "", stdout: JSON.stringify({ version: 6, request_id: "hello", ok: true, result: { manager_version: "webui-0.1.0", compatible: false, daemon_protocol_min: 1, daemon_protocol_max: 1, daemon_schema_min: 1, daemon_schema_max: 1, active_schema_version: 1, supported_operations: [], supported_features: [] } }) } } });
    const session = new EventSession({ host, kinds: ["runtime"], managerVersion: "webui-0.1.0" });
    session.start();
    await vi.runAllTimersAsync();
